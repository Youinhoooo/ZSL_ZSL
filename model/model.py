...
# 鉴别器：衡量X与生成的Xp、S与生成的Sp的相似度，用于训练
# 在对抗网络中，生成器的目标之一是最小化鉴别器的loss,从而生成更逼真的样本；回归器的目标之一也是最小化鉴别器的loss，让X更准确地映射到S
class Discriminator(nn.Module):
    def __init__(self, x_dim=2048, s_dim=312, layers='1200 600'):
        ...
        # fcn_layers.append(nn.Linear(num_hidden, 1))  # 添加最后一层线性层，输出维度为1
    ...
    def dis_loss(self, *, X, Xp, S, Sp):
        ...
        return self.mse_loss(true_scores, torch.ones_like(true_scores).to(device)) + self.mse_loss(fake_scores,
                                                                                                   torch.zeros_like(
                                                                                                       fake_scores).to(
                                                                                                       device)) + self.mse_loss(
            ctrl_socres, torch.zeros_like(ctrl_socres).to(device))
